...
- [ ] Parity batch 3 remaining: pan when zoomed, focus control (Focus* nodes exist on the A70), camera NUC/range events → events.json, DeviceTemperature per run, PTP, Research Studio validation table (needs a session with Research Studio), installer clean-machine runs (Mac/Windows/Linux) once the SDK mirror release is published by the owner.
