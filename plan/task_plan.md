...
- [ ] A70 hardware features to exploit (from probe_output_a70_templinear node list): NUCMode=Manual during a recording + NUCAction before Record (no mid-run freeze); EventNUCStart/End + EventRangeSwitch camera events → exact NUC/range marks in events.json; DeviceTemperature (FPA/housing) logged per run to explain drift; ChunkData (per-frame camera timestamp/frame id in the payload); PtpEnable for host-camera clock sync; TriggerMode/TriggerSource=Line0 hardware trigger + LineSource digital output; Focus* nodes (FocusPos/AutoFocus/FocusDistance — motorized focus exists) → focus control in the camera panel; GevSCPSPacketSize jumbo frames; StreamBufferCount* host buffers. No on-camera storage/"arm memory" exists on the A70 GigE stream.
