...
- [x] ResearchIR-manual parity batch (2026-09-02 late): periodic recording (every Nth frame), scale from selected ROI, max/min hold, saturation warnings, flip H/V, ROI set save/load JSON. Manual reviewed: ResearchIR 4 User's Guide (Research Studio's predecessor); still absent and camera-free: 3×3 measurement-cursor spot, ellipse/freehand/bendable-line ROIs, plateau-equalisation AGC, segmentation range for stats, image filters (Gaussian/median), minimap, frame-range export (TIFF stack / MATLAB .mat), radiance/counts units, PDF report.
