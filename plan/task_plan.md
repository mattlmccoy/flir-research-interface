...
- [x] Parity batch 2 (2026-09-02): line profile + histogram section, ROI delta trace, NUC marks on plots, RF hotkeys (r/f).
- [x] Operator on the dev Mac is now a launchd login item (installed 2026-09-02 via `fri-install` for this checkout; KeepAlive verified: killed pid came back in 1 s). Do NOT start a second operator on :8000 from Claude sessions; use `launchctl kickstart -k …` to restart after backend changes.
- [~] M10 installers (2026-09-02 late): install.sh (macOS + Linux branch), install.ps1 (Windows), all fetch the Spinnaker SDK + PySpin from the internal GitHub release `sdk-4.4.0.246` (FRI_SDK_BASE_URL overrides). BLOCKED: the release upload must be run by the owner (`gh release create/upload`, see docs/installation.md §0); the macOS DMG and the Windows/Linux-x86_64 PySpin wheels are not on this Mac and must be downloaded once from Teledyne. None of the three scripts has run on a clean machine yet.
- [x] NUC hold (2026-09-02): NUC before Record / at arm, NUCMode=Off during, restored after. Real-A70 check pending (enum value 'Off' assumed from the simulated backend's list; verify the A70's NUCMode entries).
- [ ] A70 hardware features still to exploit (from probe_output_a70_templinear node list): EventNUCStart/End + EventRangeSwitch camera events → exact NUC/range marks in events.json; DeviceTemperature (FPA/housing) logged per run to explain drift; ChunkData (per-frame camera timestamp/frame id in the payload); PtpEnable for host-camera clock sync; TriggerMode/TriggerSource=Line0 hardware trigger + LineSource digital output; Focus* nodes (FocusPos/AutoFocus/FocusDistance — motorized focus exists) → focus control in the camera panel; GevSCPSPacketSize jumbo frames; StreamBufferCount* host buffers. No on-camera storage/"arm memory" exists on the A70 GigE stream.
- [x] Studio layout pass (2026-09-02 late): pop-out rail sections, 400 px rail, one-line ROI rows + stats line, dock restore bar, transport under the plot; reference-frame subtraction (diverging palette).
- [x] Pan when zoomed, transport survives dock collapse, device temperature at start/stop, save image (PNG with overlay + caption), run sizes + total on the experiments page, jump to prev/next mark (2026-09-02 late). SDK mirror release complete: 11 assets incl. Spinnaker-4.4.0.246.dmg and the Windows PySpin zip (no Linux x86_64 PySpin exists for cp312).
- [x] Palettes (2026-09-02 late): inferno/magma/plasma/viridis/cividis/turbo from matplotlib's tables, CIE L* uniformity test; FLIR-style rainbow-hc; notes in the picker.
- [x] ResearchIR-manual parity batch (2026-09-02 late): periodic recording (every Nth frame), scale from selected ROI, max/min hold, saturation warnings, flip H/V, ROI set save/load JSON. Manual reviewed: ResearchIR 4 User's Guide (Research Studio's predecessor); DONE since: 3×3 measurement cursor, ellipse ROI, plateau-equalisation AGC, segmentation, blur/median filters, frame-range export (zip / TIFF stack), PDF report. Still absent: freehand + bendable-line ROIs, minimap when zoomed, radiance/counts display units, MATLAB .mat (HDF5 covers it).
- [x] Project profile (2026-09-02 late): metadata fields + mark buttons/hotkeys are no longer hard-coded RF items; stored on the operator (calibration/profile.json), edited on the setup page, stamped into metadata.json. RF heating preset available. Analysis tab shelved: ROIs are not comparable across runs without a fixed mount/fiducial, and the tool must stay experiment-agnostic (mockup: claude.ai artifact a6b104cf).
- [x] Derived outputs upgraded (2026-09-02 night): peak_frame(.png/_rois.png) at 2x native with inferno + robust scale + colour bar + caption; roi_plot 2400x1000; thermal_preview(.mp4/_rois.mp4) at 2x, CRF 27 (~3 MB per 40 s); exports/ listing + download in playback. Regenerated for all existing runs.
- [ ] Parity batch 3 remaining: focus control (Focus* nodes exist on the A70), camera NUC/range events → events.json, PTP, Research Studio validation table (needs a session with Research Studio), installer clean-machine runs (Mac/Windows/Linux) once the SDK mirror release is published by the owner.
- [~] M11 Armed recording with triggers (2026-09-02): backend done — `recording/trigger.py` state machine, `recording/arm.py` controller (pre-trigger ring, ordered flush), `POST /api/recording/arm|disarm|arm/start`, `armed` status, `trigger`/`trigger_end` events; verified on the simulated camera. Studio UI done (record panel → "trigger…" → Arm; ARMED/TRIGGERED badge with the watched value; start now; stop & disarm). NOT yet exercised on the A70. Hardware idea: a witness resistor on the RF keying line as the watched ROI. Design, to be TDD'd as a pure
