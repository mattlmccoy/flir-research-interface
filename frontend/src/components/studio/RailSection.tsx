...
interface Props { title: string; open: boolean; onToggle: () => void; tag?: string; id?: Section; children: ReactNode; }

/** Collapsible rail section; with an `id` it can pop out into a floating window (⧉) and dock back. */
export function RailSection({ title, open, onToggle, tag, id, children }: Props) {
  const fl = useFloat();
