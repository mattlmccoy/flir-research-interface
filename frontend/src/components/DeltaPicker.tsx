...
export function DeltaPicker({ rois, delta, onChange }: Props) {
  ...
  return (
    <span className="row" style={{ gap: 4, alignItems: "center" }} title="Plot the difference between two ROIs (mean, or value for spots) as an extra trace">
      <label className="hint"><input type="checkbox" checked={!!delta} onChange={(e) => onChange(e.target.checked ? { a, b } : null)} /> Δ</label>
      <select value={a} disabled={!delta} aria-label="delta A" onChange={(e) => onChange({ a: Number(e.target.value), b })}>{rois.map(opt)}</select>
      <span className="hint">−</span>
      <select value={b} disabled={!delta} aria-label="delta B" onChange={(e) => onChange({ a, b: Number(e.target.value) })}>{rois.map(opt)}</select>
    </span>
  );
}
