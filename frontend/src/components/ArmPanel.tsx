...
/** The "armed trigger" recording mode: pick a start and an end condition, arm, and let the trigger
 *  run the recording. When armed it shows the live armed/triggered status instead of the form. */
export function ArmPanel({ rois, armed, recording, disabled, busy, onArm, onDisarm, onStartNow }: Props) {
  ...
  if (armed) {
    ...
    return (
      <div className="warnbox" role="status" style={{ display: "grid", gap: 6 }}>
        ...
        <div className="row">
          {!recording && <button className="primary" disabled={busy} onClick={onStartNow} title="Start recording now regardless of the start condition">start now</button>}
  ...
  // The trigger form (shown when the operator has chosen the "armed trigger" mode).
  const needsRoi = f.startKind === "threshold" || f.endKind === "threshold";
  return (
    <div className="kv" style={{ alignItems: "center" }}>
      ...
      <span className="v plain" style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
        <select value={f.startKind} aria-label="start condition" onChange={(e) => upd({ startKind: e.target.value as TriggerForm["startKind"] })}>
          <option value="manual">manually</option><option value="rf">when RF turns on</option><option value="after">after a delay</option><option value="threshold">when a temperature crosses</option>
        </select>
        ...
        {f.startKind === "threshold" && <>
          ...
          <NumberField step={0.5} value={f.level} style={{ width: 64 }} aria-label="start level °C" onChange={(n) => upd({ level: n })} /><span className="hint">°C</span>
        </>}
      </span>
      <span>stop</span>
      <span className="v plain" style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
        <select value={f.endKind} aria-label="end condition" onChange={(e) => upd({ endKind: e.target.value as TriggerForm["endKind"] })}>
          <option value="manual">manually</option><option value="rf">when RF turns off</option><option value="duration">after a duration</option><option value="frames">after N frames</option><option value="threshold">when a temperature crosses</option>
        </select>
      ...
      {adv && <>
        ...
        {needsRoi && <>
          <span>held for</span>
          <span className="v plain"><NumberField min={1} step={1} value={f.sustain} style={{ width: 48 }} aria-label="sustain frames" onChange={(n) => upd({ sustain: Math.max(1, n) })} /> <span className="hint">frames (rejects a one-frame spike)</span></span>
        </>}
        <span>safety cap</span>
        <span className="v plain"><NumberField min={1} step={60} value={f.maxSeconds} style={{ width: 72 }} aria-label="max seconds" onChange={(n) => upd({ maxSeconds: Math.max(1, n) })} /> <span className="hint">s — always stops the run</span></span>
      </>}

      <button className="primary" style={{ gridColumn: "1 / -1", justifySelf: "start" }} disabled={disabled || busy} onClick={() => onArm(spec)} title={triggerSummary(spec)}>◌ Arm this trigger</button>
    </div>
  );
}
