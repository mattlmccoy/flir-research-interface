...
|---|---|---|
| ROI series CSV | `GET /api/experiments/{name}/export/series.csv?rois=…` (playback rail → export) | `t_s, frame_id, S<n>_value, R<n>_mean/min/max/std/n` per frame (std = population standard deviation, n = pixels counted; `valid=lo,hi` on the series endpoint applies segmentation); `#` header lines list units, the ROI geometry and any per-ROI optics (`[emissivity=…, reflected_c=…]`, see docs/radiometry.md) |
...
| PDF report | `POST …/export/report` → `exports/report.pdf`, served at `GET …/report.pdf` | README text page + ROI plot + preview + keyframes pages (rendered images) |
